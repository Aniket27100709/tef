...
def main():
    ...
    if choice == "Pandas Profile":
        ...
        fig = px.pie(df, values=df['gross income'][:500], names=df['Product'][:500], title='Gross margin of products')
        st.plotly_chart(fig)
        fig = px.pie(
            hole=0.2,
            values=df['Quantity'][:1000],
            names=df['Gender'][:1000],
        )
        st.header("Payment doers for buying percentage")
        st.plotly_chart(fig)
        fig = px.bar(
            df,
            x=df["Product"],
            y=df["Sales"],
            title="Sales of Category of Products"
        )
        st.plotly_chart(fig)
